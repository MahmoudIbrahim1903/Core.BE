...
using VictoryLinkResellerServiceReference;

namespace Emeint.Core.BE.SMS.Domain.SmsProviders
{
    public class VictoryLinkResellerSmsProvider : ISmsSender
    {
        private readonly IConfigurationManager _configurationManager;
        private readonly IIdentityService _identityService;
        public VictoryLinkResellerSmsProvider(IConfigurationManager configurationManager, IIdentityService identityService)
        {
        ...
        public Task<int> SendSmsAsync(string number, string message)
        {
            ...
            var client = new SMSSenderSoapClient(SMSSenderSoapClient.EndpointConfiguration.SMSSenderSoap);
            //var result = client.SendSMSAsync(smsUserName, smsPassword, message, language, smsSender, number).Result;
            var result = client.SendSMSWithDLRAsync(smsUserName, smsPassword, message, language, smsSender, number, Guid.NewGuid().ToString(), null, null).Result;
            switch (result)
            {
                case -1:
                ...
                case -13:
                    throw new SendSmsFailedException(number, "Invalid Sender With Connection.", -13);
                case -25:
