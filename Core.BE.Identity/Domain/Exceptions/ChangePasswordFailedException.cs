...
namespace Emeint.Core.BE.Identity.Domain.Exceptions
{
    ...
    {
        public ChangePasswordFailedException(List<IdentityError> errors, int passwordMinLength)
        {
            IdentityErrorDescriber identityErrorDescriber = new IdentityErrorDescriber();

            Code = (int)IdentityErrorCodes.ChangePasswordFailed;
            Resourcekey = IdentityErrorCodes.ChangePasswordFailed.ToString();
            ...
            foreach (var error in errors)
            {
               ...
               /* if (MessageEn != string.Empty)
                ...
                if (error.Code == identityErrorDescriber.PasswordMismatch().Code)
                {
                    MessageEn += "Incorrect Password";
                    MessageAr += "كلمة المرور غير صحيحة";
                }
                else if (error.Code == identityErrorDescriber.PasswordRequiresDigit().Code)
                {
                ...
                else if (error.Code == identityErrorDescriber.PasswordTooShort(passwordMinLength).Code)
                {
                    ...
                    MessageAr += $"يجب ألا تقل كلمة المرور عن {passwordMinLength} حروف ";
                }
                else
                {
